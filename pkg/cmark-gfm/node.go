...
// GetType wraps cmark_node_get_type.
// Returns the type of 'node' or [NodeTypeNone] on failure
func (node *Node) GetType() NodeType {
	return NodeType(C.cmark_node_get_type(node.node))
}

// GetTypeString wraps cmark_get_type_string_func.
// Like GetType but returns a string representation of the
// type, or `"<unknown>"`
